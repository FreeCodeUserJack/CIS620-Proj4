...
//BUG: need to link math library, stdc++ library, and cudart library
...
__global__ void
mapqroot(double *g_idata, double *g_odata) {
...
extern "C" double reduction(int n, int mean, int seed) {
        ...
        mapqroot<<<numBlocks, maxThreads, smemSize>>>(d_idata, d_idata);
